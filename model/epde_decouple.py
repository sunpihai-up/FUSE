...
from model.epde.decouple import (
    Decouple_Image,
    Feature_Decouple_Fusion,
    GaussianLaplacianPyramid,
    SpatialFrequency,
    PatchGra,
    Direction_Var_Entropy,
)
...
class EPDEVisionTransformer(nn.Module):
    def __init__(
        ...
        self.img_decouple = Decouple_Image()
        self.vox_decouple = Decouple_Image(alpha=0.5, beta=0.2, lambda_max=500)

        self.low_weight = Direction_Var_Entropy()
        ...
        # self.feature_fusion = Feature_Decouple_Fusion(embed_dim=embed_dim)
        self.feature_fusion = Feature_Decouple_Fusion(embed_dim=embed_dim, num_heads=self.num_heads)
    ...
    def init_from_pretrained_weight(self):
        ...
        if has_lora:
            ...
            print(f"Loaded pretrained prompt weights without lora. ")
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1):
        ...
        # img_low, img_high = self.img_decouple(image)
        # vox_low, vox_high = self.vox_decouple(event)

        # img_low_weight = self.low_weight(img_low)
        # vox_low_weight = self.low_weight(vox_low)

        # img_high_weight = self.high_weight(img_high)
        # vox_high_weight = self.high_weight(vox_high)
        ...
        for i in range(self.depth):
            ...
            if i in self.blocks_to_take:
                ...
                fuse_token = self.feature_fusion(
                    img_read_out,
                    pro_read_out,
                    # img_low_weight,
                    # vox_low_weight,
