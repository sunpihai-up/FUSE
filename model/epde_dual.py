...
class EPDEVisionTransformer(nn.Module):
    def __init__(
        ...
        self.img_cof = ModalConfidence()
        self.vox_cof = ModalConfidence(alpha=0.5, beta=0.2, lambda_max=500)
    ...
    def init_from_pretrained_weight(self):
        ...
        if has_lora:
            print(f"Loaded pretrained image encoder weights with lora!")
        else:
            print(f"Loaded pretrained image encoder weights without lora!")
        ...
        if has_lora:
            print(f"Loaded pretrained prompt encoder weights with lora!")
        else:
            print(f"Loaded pretrained prompt encoder weights without lora!")
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1):
        ...
        img_low_weight, img_high_weight = self.img_cof(image)
        vox_low_weight, vox_high_weight = self.vox_cof(event)
